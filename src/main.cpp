		...
		if (length && length > 2 && data[0] == '4' && data[1] == '2')
		{
			...
			if (s != "")
			{
				...
				if (event == "telemetry")
				{
					...
					int previos_size = previous_path_x.size();
					...
					double ref_x = car_x;
					double ref_y = car_y;
					double ref_yaw = rad2deg(car_yaw);
					...
					{
						ref_x = previous_path_x[previos_size - 1];
						ref_y = previous_path_y[previos_size - 1];
						double ref_x_prev = previous_path_x[previos_size - 2];
						double ref_y_prev = previous_path_y[previos_size - 2];
					...
					{
						double d_x = ptsx[i] - ref_x;
						double d_y = ptsy[i] - ref_y;

						ptsx[i] = (d_x * cos(-ref_yaw) - d_y * sin(-ref_yaw));
						ptsy[i] = (d_x * sin(-ref_yaw) + d_y * cos(-ref_yaw));
					}
